...
public class ChatPatchMoveLast {
    public static void main(String[] args) throws Exception {
        ...
        try {
            CtField f = CtField.make("public static java.awt.Component tmpComboRefLast = null;", cc);
            cc.addField(f);
        ...
        m.instrument(new javassist.expr.ExprEditor() {
            public void edit(javassist.expr.NewExpr e) throws CannotCompileException {
                if (e.getClassName().equals("javax.swing.JComboBox") && e.getSignature().equals("()V")) {
                    e.replace("{ $_ = $proceed($$); com.group_finity.mascot.plugin.MultiSelectPlugin.tmpComboRefLast = $_; }");
                }
            }
            ...
            public void edit(javassist.expr.MethodCall mc) throws CannotCompileException {
                if (mc.getClassName().equals("javax.swing.JPanel") && mc.getMethodName().equals("add") && mc.getSignature().equals("(Ljava/awt/Component;Ljava/lang/Object;)V")) {
                    ...
                    mc.replace("{ if ($1 instanceof javax.swing.JButton && ((javax.swing.JButton)$1).getText() != null && ((javax.swing.JButton)$1).getText().contains(\\"模型仓库\\")) { if (com.group_finity.mascot.plugin.MultiSelectPlugin.tmpComboRefLast != null) { javax.swing.JComboBox cb = (javax.swing.JComboBox)com.group_finity.mascot.plugin.MultiSelectPlugin.tmpComboRefLast; cb.setPreferredSize(new java.awt.Dimension(200, 26)); cb.setOpaque(true); cb.putClientProperty(\\"JComboBox.buttonType\\", \\"default\\"); $0.add(cb); } } $_ = $proceed($$); }");
                }
                    ...
                    mc.replace("{ if ($1 == false && $0 == com.group_finity.mascot.plugin.MultiSelectPlugin.tmpComboRefLast) { /* do nothing */ } else { $proceed($$); } }");
                }
                else if (mc.getClassName().equals("javax.swing.JComboBox") && mc.getMethodName().equals("setBorder")) {
                    mc.replace("{ if ($0 == com.group_finity.mascot.plugin.MultiSelectPlugin.tmpComboRefLast) { /* do nothing */ } else { $proceed($$); } }");
                }
            }
        });
        
        cc.writeFile("/tmp/shimeji_layout_last_out");
        System.out.println("Saved modified MultiSelectPlugin with properly styled ComboBox");
    }
}
"""
os.system("mkdir -p /tmp/shimeji_layout_last_out")
with open("/tmp/shimeji_layout_last/ChatPatchMoveLast.java", "w") as f:
    f.write(javassist_code)

subprocess.run(["javac", "-cp", "/tmp/javassist.jar", "/tmp/shimeji_layout_last/ChatPatchMoveLast.java"])
subprocess.run(["java", "-cp", "/tmp/shimeji_layout_last:/tmp/javassist.jar", "ChatPatchMoveLast"])
